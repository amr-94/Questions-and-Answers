...
@section('content')
    ...
    <form method="post" action="{{ route('questions.update', $question->id) }}" enctype="multipart/form-data">
        ...
        <select class="form-select" aria-label="Default select example" name="status">
            ...
            <option value="open" @if ($question->status == 'open') selected @endif> open</option>
            <option value="close" @if ($question->status == 'close') selected @endif> close</option>
        </select>
            ...
            @foreach ($tags as $tags)
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" value="{{ $tags->id }}" name="tags[]"
                        id="{{ $tags->id }}" @if (in_array($tags->id, $question_tag)) checked @endif>
                    @error('tags')
                        <p class="invalid-feedback">{{ $message }}</p>
                    @enderror
